...
async function loadGLTF(path) {
  // Strings only (repo paths). Local File objects removed for simplicity.
...
export async function loadWingsMesh(path) {
  const gltf = await loadGLTF(path);
...
export async function loadModelsConfig() {
  ...
  throw new Error("models.json not found from this page.");
}
