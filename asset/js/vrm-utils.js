// Patched vrm-utils.js — robust models.json lookup for all pages (index, pages/*, pages/rooms/*)
...
// NEW: robust models.json finder that works from any page depth.
export async function loadModelsConfig() {
  // Prefer local override saved by Wing Room
  const local = localStorage.getItem("ct.models.json");
  if (local) return JSON.parse(local);

  const candidates = [
    "../../data/models.json", // rooms/*
    "../data/models.json",    // pages/*
    "./data/models.json"      // index
  ];
  ...
  // Friendly on-screen hint so blank pages aren’t silent
  showHUD("Couldn’t find <code>data/models.json</code>. Make sure it exists and paths are correct.");
  throw new Error("models.json not found from this page. Tried: " + candidates.join(", "));
}

// Tiny HUD for user-facing error hints
